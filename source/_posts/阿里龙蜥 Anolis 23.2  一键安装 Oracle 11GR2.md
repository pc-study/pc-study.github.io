---
title: 阿里龙蜥 Anolis 23.2  一键安装 Oracle 11GR2
date: 2025-03-05 10:48:41
tags: [墨力计划]
author: 三笠丶
source: 墨天轮
source_url: https://www.modb.pro/db/1897117027182063616
---

>大家好，这里是 **Lucifer三思而后行**，专注于提升数据库运维效率。

## 前言
Oracle 一键安装脚本，演示 龙蜥 Anolis 23.2 一键安装 Oracle 11GR2 单机（全程无需人工干预）。
...
```bash
# 主机版本
[root@anolis23 soft]# cat /etc/os-release 
NAME="Anolis OS"
VERSION="23.2"
ID="anolis"
VERSION_ID="23.2"
...
# 网络信息
[root@anolis23 soft]# ip a
2: ens33: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc fq_codel state UP group default qlen 1000
    link/ether 00:50:56:a3:5d:b7 brd ff:ff:ff:ff:ff:ff
    altname enp2s1
    inet 192.168.6.120/24 brd 192.168.6.255 scope global noprefixroute ens33
       valid_lft forever preferred_lft forever
    inet6 fe80::250:56ff:fea3:5db7/64 scope link noprefixroute 
       valid_lft forever preferred_lft forever

# 挂载本地 ISO 镜像
[root@anolis23 soft]# mount | grep iso9660 | grep -v "/run/media"
/dev/sr0 on /mnt type iso9660 (ro,relatime,nojoliet,check=s,map=n,blocksize=2048,iocharset=utf8)
[root@anolis23 soft]# df -h | grep /mnt
/dev/sr0                     15G   15G     0  100% /mnt

# 安装包存放在 /soft 目录下
[root@anolis23 soft]# ll
-rwxr-xr-x. 1 root root     244843  3月 4日 15:27 OracleShellInstall
-rwx------. 1 root root 1395582860  3月 4日 15:48 p13390677_112040_Linux-x86-64_1of7.zip
-rwx------. 1 root root 1151304589  3月 4日 15:48 p13390677_112040_Linux-x86-64_2of7.zip
-rwx------. 1 root root     321590  3月 4日 15:48 rlwrap-0.44.tar.gz
```
...
!!! 免责声明：当前操作系统版本是 [ Anolis OS 23.2 ] 不在 Oracle 官方支持列表，本脚本只负责安装，请确认是否继续安装 (Y/N): [Y] 

OracleShellInstall 开始安装，详细安装过程可查看日志： tail -2000f /soft/print_shell_install_20250305095049.log                                                                                  
...
正在解压 Oracle 安装包以及补丁......已完成 (耗时: 17 秒)
正在安装 Oracle 软件以及补丁......已完成 (耗时: 183 秒)
正在创建监听......已完成 (耗时: 2 秒)
正在创建数据库......已完成 (耗时: 183 秒)
正在优化数据库......已完成 (耗时: 13 秒)

恭喜！Oracle 一键安装执行完成 (耗时: 451 秒)，现在是否重启主机：[Y/N] Y
...
查看系统版本：
```bash
[root@anolis23:/root]# cat /etc/os-release
NAME="Anolis OS"
VERSION="23.2"
ID="anolis"
VERSION_ID="23.2"
...
[oracle@anolis23:/home/oracle]$ opatch lspatches
此 Oracle 主目录中未安装任何中间补丁程序。
```
连接数据库：
```bash
[oracle@anolis23:/home/oracle]$ sas

SQL*Plus: Release 11.2.0.4.0 Production on Wed Mar 5 10:23:03 2025
