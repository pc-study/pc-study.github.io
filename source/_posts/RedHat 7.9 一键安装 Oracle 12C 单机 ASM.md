---
title: RedHat 7.9 一键安装 Oracle 12C 单机 ASM
date: 2024-05-24 16:04:58
tags: [墨力计划]
author: 三笠丶
source: 墨天轮
source_url: https://www.modb.pro/db/1793915997920514048
---

# 前言

Oracle 一键安装脚本，演示 RedHat 7.9 一键安装 Oracle 12C 单机 ASM 过程（全程无需人工干预）。
...
```bash
# 主机版本
[root@rac soft]# cat /etc/os-release 
...
# 挂载本地 ISO 镜像
[root@rac soft]# mount | grep iso9660 | grep -v "/run/media"
...
# 安装包存放在 /soft 目录下
[root@rac soft]# ll
-rwx------. 1 root root 3453696911 Apr 28 12:34 LINUX.X64_122010_db_home.zip
-rwx------. 1 root root 2994687209 Apr 28 12:34 LINUX.X64_122010_grid_home.zip
-rwxr-xr-x. 1 root root     199185 Apr 28 12:32 OracleShellInstall
-rwx------. 1 root root 2526745297 Apr 28 12:33 p35745595_122010_Linux-x86-64.zip
-rwx------. 1 root root  138325588 Apr 28 12:33 p35926712_122010_Linux-x86-64.zip
-rwx------. 1 root root  127629034 Apr 28 12:32 p6880880_122010_Linux-x86-64.zip
...
OracleShellInstall 开始安装，安装过程可查看日志： /soft/print_ora_install_20240428123542.log                                                                                  
...
正在解压 Grid 安装包以及补丁......已完成 (耗时: 247 秒)
正在解压 Oracle 安装包以及补丁......已完成 (耗时: 95 秒)
正在安装 Grid 软件以及补丁......已完成 (耗时: 993 秒)
正在安装 Oracle 软件以及补丁......已完成 (耗时: 1023 秒)
正在创建数据库......已完成 (耗时: 740 秒)
正在优化数据库......已完成 (耗时: 121 秒)

恭喜！Oracle 单机 ASM 安装成功 (耗时: 3339 秒)，现在是否重启主机：[Y/N] Y
...
```bash
[grid@rac:/home/grid]$ sqlplus -v

SQL*Plus: Release 12.2.0.1.0 Production

[grid@rac:/home/grid]$ opatch lspatches
...
```bash
[grid@rac:/home/grid]$ crsctl stat res -t
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
ora.DATA.dg
               ONLINE  ONLINE       rac                      STABLE
ora.LISTENER.lsnr
               ONLINE  ONLINE       rac                      STABLE
ora.asm
               ONLINE  ONLINE       rac                      Started,STABLE
ora.ons
               OFFLINE OFFLINE      rac                      STABLE
--------------------------------------------------------------------------------
Cluster Resources
--------------------------------------------------------------------------------
ora.cssd
      1        ONLINE  ONLINE       rac                      STABLE
ora.diskmon
      1        OFFLINE OFFLINE                               STABLE
ora.evmd
      1        ONLINE  ONLINE       rac                      STABLE
ora.orcl.db
      1        ONLINE  ONLINE       rac                      Open,HOME=/u01/app/o
...
```bash
[oracle@rac:/home/oracle]$ sqlplus -v

SQL*Plus: Release 12.2.0.1.0 Production

[oracle@rac:/home/oracle]$ opatch lspatches
...
```bash
[oracle@rac:/home/oracle]$ sas

SQL*Plus: Release 12.2.0.1.0 Production on Sun Apr 28 14:12:42 2024
...
sys@ORCL 2024-04-28 14:12:43> show parameter name
