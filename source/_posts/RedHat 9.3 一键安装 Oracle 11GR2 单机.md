---
title: RedHat 9.3 一键安装 Oracle 11GR2 单机
date: 2024-06-14 15:45:44
tags: [墨力计划]
author: 三笠丶
source: 墨天轮
source_url: https://www.modb.pro/db/1801521277945057280
---

# 前言

Oracle 一键安装脚本，演示 RedHat 9.3 一键安装 Oracle 11GR2 单机过程（全程无需人工干预）。
...
```bash
# 主机版本
[root@rhel9 soft]# cat /etc/os-release 
...
# 挂载本地 ISO 镜像
[root@rhel9 soft]# mount | grep iso9660 | grep -v "/run/media"
/dev/sr0 on /mnt type iso9660 (ro,relatime,nojoliet,check=s,map=n,blocksize=2048)
[root@rhel9 soft]# df -h|grep /mnt
/dev/sr0               9.9G  9.9G     0 100% /mnt

# 安装包存放在 /soft 目录下
[root@rhel9 soft]# ll
-rwxr-xr-x. 1 root root     214424 Jun 13 15:48 OracleShellInstall
...
OracleShellInstall 开始安装，详细安装过程可查看日志： tail -2000f /soft/print_shell_install_20240613163821.log                                                                                  
正在获取操作系统信息......已完成 (耗时: 1 秒)
正在配置本地软件源......已完成 (耗时: 1 秒)
正在安装依赖包......已完成 (耗时: 276 秒)
正在禁用防火墙......已完成 (耗时: 1 秒)
...
正在安装 Oracle 软件以及补丁......已完成 (耗时: 385 秒)
正在创建监听......已完成 (耗时: 14 秒)
正在创建数据库......已完成 (耗时: 260 秒)
正在优化数据库......已完成 (耗时: 39 秒)

恭喜！Oracle 一键安装执行完成 (耗时: 1029 秒)，现在是否重启主机：[Y/N] Y
...
检查监听：
```bash
[oracle@rhel9:/home/oracle]$ lsnrctl stat
